...
env = gym.make("Gambler-v0")

class MDP:

    def __init__(self, environment, convergence_threshold=0.00001, gamma=0.9, max_iterations=100, grid=False):
        ...
        while iteration <= self.max_iterations and delta >= self.convergence_threshold:
            ...
            for state in range(self.env.nS):
                ...
                if policy is not None:
                    ...
                    V[state] = 0. # Contains optimal states 
                    # print('\n---> V before: {}'.format(self.optimal_V))
                    for action in range(self.env.nA):
                        V[state] += policy[state, action] * Q_current_state[action]
                    # print('\n---> V after: {}'.format(self.optimal_V))
                ...
                if visualize and policy is None and iteration == iterations_to_save[saving_index % len(iterations_to_save)]:
                    # print('saving_index',saving_index)
    ...
    def policy_iteration(self, iterations_to_save=[0], visualize=False):
        ...
        while iteration <= self.max_iterations and improve_policy:
            ...
            # Evaluate current policy and finds on optimal_V
        ...
        while True:
            ...
            while True:
                ...
                if np.random.random() < epsilon:
                    ...
                    action= np.argmax(Q[state].round(decimals=2))
    ...
    def value_iteration(self, iterations_to_save=[1], visualize=False):
        ...
        for state in range(self.env.nS):
            ...
            # print('state and best_action:',best_action)
...
if __name__ == '__main__':
    ...
    frozen_lake = MDP(environment='FrozenLake-v0', convergence_threshold=0.0001, grid=False)
    print('Frozen Lake: ')
    frozen_lake.value_iteration(iterations_to_save=[i for i in range(1,100,5)], visualize=False)
    frozen_lake.policy_iteration(iterations_to_save=[0, 1, 2], visualize=True)

    frozen_lake.Q_learning(num_episodes=10000, learning_rate_decay=0.995, epsilon_decay=0.995, visualize=False)
